  ...
  return (
    <section className="container-px mx-auto max-w-7xl py-20">
      <SectionHeading
        ...
        action={<Link to="/knowledge" className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:underline">All articles <ArrowRight className="h-4 w-4" /></Link>}
      />
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          {articles.map((a, i) => (
            <article key={a.id} className={`group flex gap-5 rounded-3xl border border-border bg-card p-5 shadow-card transition hover:-translate-y-1 hover:shadow-elegant md:p-6 ${i === 0 ? "md:p-8" : ""}`}>
              ...
              <div className="flex flex-1 flex-col">
                ...
                <h3 className={`mt-2 font-extrabold text-forest group-hover:text-primary ${i === 0 ? "text-xl md:text-2xl" : "text-base md:text-lg"}`}>{a.title}</h3>
        ...
        <div className="rounded-3xl bg-forest p-6 text-forest-foreground shadow-elegant">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full bg-gold/20 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-gold">Live</span>
          </div>
          <p className="mt-1 text-xs text-forest-foreground/60">Updated every 2 hours · NCPB & KAMIS</p>
            ...
            {marketPrices.map((p) => (
              <li key={p.item} className="flex items-center justify-between py-3">
                ...
                <div className="text-right">
                  ...
                  <div className={`flex items-center justify-end gap-1 text-[11px] font-semibold ${p.change >= 0 ? "text-success" : "text-destructive"}`}>
