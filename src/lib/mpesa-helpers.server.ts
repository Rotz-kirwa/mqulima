...
import { sendOrderPaymentConfirmedSmsById } from "./order-sms.server";
...
export async function handleMpesaCallback(payload: any, request?: Request) {
  ...
  await sql.begin(async (tx: any) => {
    ...
    if (ResultCode === 0) {
      ...
      // Fire Payment Confirmation SMS asynchronously (non-blocking) with full item names and price breakdown
      const refCode = receiptNumber || CheckoutRequestID;
      sendOrderPaymentConfirmedSmsById(currentPayment.order_id, refCode).catch((err) =>
        console.error("[M-PESA CALLBACK] Payment SMS dispatch error:", err)
      );
    } else {
