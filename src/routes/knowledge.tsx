      ...
      <div className="bg-sepia text-sepia-foreground min-h-screen font-sans selection:bg-primary/20 selection:text-primary">
        ...
        <section className="bg-sepia-muted/30 border-b border-sepia-muted/40 py-16 text-sepia-foreground">
          <div className="container-px mx-auto max-w-7xl">
            <span className="inline-flex items-center gap-2 rounded-full bg-sepia-muted/65 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-primary">Knowledge Hub</span>
            <h1 className="mt-3 text-4xl font-extrabold md:text-5xl text-forest">Smart farming, simply explained.</h1>
            <p className="mt-2 max-w-xl text-sepia-foreground/80">Practical guides, weekly market prices and answers from real agronomists.</p>
          </div>
        </section>
            ...
            <div className="grid gap-8 lg:grid-cols-[2fr_1fr]">
              <div className="grid gap-4 sm:grid-cols-2">
                {[...articles, ...articles].slice(0, 6).map((a, i) => (
                  <article key={i} className="group overflow-hidden rounded-3xl border border-sepia-muted/60 bg-card shadow-card transition hover:-translate-y-1 hover:shadow-elegant">
              ...
              <aside className="space-y-6">
                    ...
                    {marketPrices.map((p) => (
                      <li key={p.item} className="flex items-center justify-between py-3">
                        ...
                        <div className="text-right">
                          <div className="text-sm font-extrabold text-gold font-mono">KES {p.price.toLocaleString()}</div>
                          <div className={`flex items-center justify-end gap-1 text-[10px] ${p.change >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                ...
                <div className="rounded-3xl bg-gradient-to-br from-gold to-amber-400 p-6 text-gold-foreground shadow-gold">
                  ...
                  <p className="mt-1 text-sm text-gold-foreground/90">Submit your question and get an expert answer within 24 hours.</p>
                  <Link 
                    to="/contact" 
