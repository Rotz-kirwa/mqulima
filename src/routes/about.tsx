      ...
      <section className="bg-gradient-to-br from-forest to-primary py-20 text-forest-foreground">
        <div className="container-px mx-auto max-w-4xl text-center">
          ...
          <h1 className="mt-4 text-4xl font-extrabold md:text-6xl">
            Built in Eldoret, for every Kenyan farmer.
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-lg text-forest-foreground/85">
            Mqulima exists because farming in Kenya deserves better tools. We're a team of
            agronomists, vets and technologists obsessed with one outcome: making farmers richer.
          </p>
        </div>
      </section>
          ...
          {[
            {
              icon: Target,
              title: "Our Mission",
              text: "Give every Kenyan farmer instant access to quality inputs, expert services and intelligent farm advice.",
            },
            {
              icon: Heart,
              title: "Our Values",
              text: "Farmer-first. Honest pricing. Same-day delivery. Receipts for everything. No middlemen tax.",
            },
            {
              icon: Award,
              title: "Our Promise",
              text: "If a Mqulima product doesn't perform as advertised, we replace it or refund — full stop.",
            },
          ].map((c) => (
        ...
        <div className="mt-20">
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {team.map((m) => (
              <div
                key={m.name}
                className="group rounded-3xl border border-border bg-card p-6 text-center shadow-card transition hover:-translate-y-1 hover:shadow-elegant"
              >
                <img
                  src={m.image}
                  alt={m.name}
