...
import { Phone, Mail, MapPin, MessageCircle, ChevronDown, Send, CheckCircle, Search, HelpCircle, FileText } from "lucide-react";
...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Reach out to Mqulima Hub. Connect via WhatsApp, phone, email, or visit our Eldoret HQ. Get expert agricultural assistance." },
    ],
  }),
  component: Contact,
});
...
function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      setSuccess(true);
      toast.success("Message sent! A customer success representative will contact you shortly.");
      // Reset form
      setName("");
      setEmail("");
      setPhone("");
      ...
      <div className="bg-background text-foreground">
        ...
        <section className="bg-gradient-to-br from-forest to-primary py-16 text-forest-foreground text-center md:text-left">
          <div className="container-px mx-auto max-w-7xl">
            ...
            <p className="mt-2 max-w-xl text-forest-foreground/80">Connect with agronomy experts, order agents, or customer service representatives 7 days a week.</p>
          </div>
        </section>
        ...
        <section className="container-px mx-auto max-w-7xl py-12">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {[
              { icon: MessageCircle, label: "WhatsApp Chat", value: "+254 711 222 333", href: "https://wa.me/254711222333", color: "text-[#25D366] bg-[#25D366]/10" },
              { icon: Phone, label: "Hotline Support", value: "+254 711 222 333", href: "tel:+254711222333", color: "text-blue bg-blue/10" },
              { icon: Mail, label: "Email Sales", value: "hello@mqulima.co.ke", href: "mailto:hello@mqulima.co.ke", color: "text-primary bg-primary/10" },
              { icon: MapPin, label: "Eldoret HQ", value: "Uganda Rd, opp. Poly", href: "#map", color: "text-gold bg-gold/10" },
            ].map((c) => (
              <a key={c.label} href={c.href} className="group flex items-start gap-4 rounded-2xl border border-border bg-card p-5 shadow-elegant transition hover:-translate-y-1 hover:border-primary/40">
                <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${c.color} transition-colors group-hover:bg-primary group-hover:text-primary-foreground`}>
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-elegant md:p-8">
              <div className="flex items-center justify-between border-b border-border pb-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">We typically reply within 15 minutes.</p>
                </div>
              ...
              {success ? (
                <div className="py-12 text-center">
                  ...
                  <h3 className="mt-4 text-base font-bold text-foreground">Thank you, message received!</h3>
                  <p className="mt-2 text-xs text-muted-foreground">Your support ticket has been registered. Reference: MQ-{Math.floor(1000 + Math.random() * 9000)}</p>
                  <button 
                ...
                <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">County</label>
                      <select 
                        value={county} 
            ...
            <div className="flex flex-col gap-6">
              <div id="map" className="overflow-hidden rounded-3xl border border-border shadow-card flex-1 min-h-[300px]">
                <iframe
              ...
              <div className="rounded-3xl border border-border bg-card p-6 shadow-elegant flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-bold text-foreground">Physical Office Headquarters</h3>
                  <p className="text-xs text-muted-foreground mt-0.5">Uganda Highway Road, Opposite Eldoret Polytechnic, Eldoret, Kenya</p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-20">
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Instant answers to common customer questions.</p>
            </div>
            ...
            <div className="mx-auto mt-6 flex max-w-lg items-center gap-2 rounded-full border border-border bg-card px-4 py-2 shadow-elegant">
              ...
              />
              {faqSearch && (
                <button onClick={() => setFaqSearch("")} className="text-muted-foreground hover:text-foreground text-xs font-bold">Clear</button>
              )}
            </div>

            <div className="mt-8 divide-y divide-border overflow-hidden rounded-3xl border border-border bg-card shadow-elegant">
              {filteredFaqs.length === 0 ? (
                <div className="p-8 text-center text-xs text-muted-foreground">No FAQs matching your search query.</div>
              ) : (
